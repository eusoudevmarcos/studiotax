...
import { CardKanban, MembroCard, UsuarioSistema } from '@/schemas/kanban.schema';
import { getUsuarioNome } from '@/utils/kanban';
import { useCallback, useEffect, useState } from 'react';
// Removido: import { FiLoader, FiUsers, FiX } from 'react-icons/fi';
...
export const MembersKanban = ({
  ...
  const getInfoMembro = (membro: MembroCard) => {
    return membro.usuarioSistema?.funcionario?.pessoa?.nome || membro.usuarioSistema?.cliente?.empresa?.nomeFantasia || membro.usuarioSistema?.cliente?.empresa?.razaoSocial || membro.usuarioSistema?.email;
  };
    ...
    return (card.membros || []).map((membro: MembroCard) => (
      ...
      >
        <div className="flex items-center">
          ...
          <span className="text-sm text-gray-700 truncate max-w-[200px] block" title={getInfoMembro(membro)}>
            {membro.usuarioSistema?.funcionario?.pessoa?.nome || membro.usuarioSistema?.cliente?.empresa?.nomeFantasia || membro.usuarioSistema?.cliente?.empresa?.razaoSocial || membro.usuarioSistema?.email}
          </span>
        </div>
        <button
          type="button"
    ...
    <>
      <div className='relative'>
        <button
          ...
          className={`flex items-center gap-2 px-2 py-1.5 rounded-md text-sm font-medium transition-colors ${activePanel === 'members'
            ...
            <div className="mb-1 autocomplete-container relative">
              ...
              {loadingUsuarios && (
                <div className="absolute right-3 top-2.5">
                  <MaterialIcon name="autorenew" className="text-gray-400" spin />
                </div>
              )}
            </div>
